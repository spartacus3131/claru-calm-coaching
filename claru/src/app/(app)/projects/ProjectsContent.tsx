...
/**
 * @file ProjectsContent.tsx
 * @description Projects screen with Active, Recurring, and Done tabs.
 * @module app/(app)/projects
 * 
 * Ported from old src/screens/ProjectsScreen.tsx for Next.js App Router.
 */
          ...
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            ...
            {project.nextActions.length > 0 && (
              <span className="flex items-center gap-1">
                ...
                {project.nextActions.length} actions
              </span>
            )}
          </div>
        </div>
        ...
        <div className="px-4 pb-4 pt-0 border-t border-border/50 space-y-3">
          ...
          {project.recentProgress && (
            <div>
              ...
              <p className="text-sm text-foreground">{project.recentProgress}</p>
            </div>
          )}
...
export function ProjectsContent() {
  ...
  const openEditDrawer = (project: Project) => {
    ...
    setFormData({
      ...
      next_actions: project.nextActions.join('\n'),
      recent_progress: project.recentProgress ?? '',
      notes: project.notes ?? ''
    });
    setDrawerOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    const projectData = {
      ...
      nextActions: formData.next_actions.split('\n').map(s => s.trim()).filter(Boolean),
      recentProgress: formData.recent_progress.trim() || null,
